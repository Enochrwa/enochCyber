...
class FeedStatus(BaseModel):
    ...
    is_subscribed: bool # Ensure this is not optional or defaulted here for clarity in response.
...
async def get_emerging_threats_endpoint():
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get emerging threats: {str(e)}")
...
async def subscribe_to_feed_endpoint(feed_id: str, subscription_request: SubscriptionRequest):
    try:
        # The service method now returns the updated feed status or an error dict
        result = threat_intelligence_service.update_feed_subscription(feed_id, subscription_request.is_subscribed)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update subscription for feed {feed_id}: {str(e)}")
...
async def refresh_feed_endpoint(feed_id: str):
    try:
        ...
        return RefreshResponse(feed_id=feed_id, status="unknown", error="Unknown result from refresh operation")

    except HTTPException as http_exc: # Re-raise HTTP exceptions
        raise http_exc
    except Exception as e:
        # logger.error(f"Unhandled exception in refresh_feed_endpoint for {feed_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"An unexpected error occurred while refreshing feed {feed_id}: {str(e)}")
